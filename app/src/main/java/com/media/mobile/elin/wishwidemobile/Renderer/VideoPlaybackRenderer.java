...
public class VideoPlaybackRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl
{
    ...
    public void renderFrame(State state, float[] projectionMatrix)
    {
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++)
        {
            ...
            {
                Log.d(LOGTAG, "저장된 마커 확인: " + md.str_name);
            ...
            if(type == 1) {
                Log.d(LOGTAG, "가상 버튼 터치" + imageTargetResult.getNumVirtualButtons());
                for (int virtual = 0; virtual < imageTargetResult.getNumVirtualButtons(); virtual++) {
                    ...
                    if (btnResult.isPressed()) {
                        Log.d(LOGTAG, "가상 버튼 누름" + imageTarget.getName());
    ...
    {
        Log.d(LOGTAG, "m_translates 값 확인: " + m_translates.length);
        for(int trans = 0;trans<m_translates.length;trans++) {
            Log.d(LOGTAG, "trans 값 확인: " + trans);
            //cpyoon
